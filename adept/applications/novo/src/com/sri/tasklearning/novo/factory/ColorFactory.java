...
public final class ColorFactory extends NovoFactory {
    public static final ColorFactory INSTANCE = new ColorFactory();        
    
    private ColorEnum selectedColor = ColorEnum.BLACK;
    ...
    private ColorFactory() {
        ...
        Label titleLabel = new Label("Color Factory");
        titleLabel.setFont(TITLE_FONT);
        title.getChildren().addAll(getIcon(ICON_SIZE),titleLabel);
        
        final SplitMenuButton colorMenu = new SplitMenuButton();

        for (ColorEnum color : ColorEnum.values()) {
            final ColorEnum finalColor = color;
            MenuItem mi = new MenuItem(color.colorName());
            
            mi.setOnAction(new EventHandler<ActionEvent>() {
                public void handle(ActionEvent e) {
                    colorMenu.setText(finalColor.colorName());
                    selectedColor = finalColor;
                    updatePreview();
                }
            });
            colorMenu.getItems().add(mi);
        }
        
        colorMenu.getItems().get(0).fire();
        ...
        inputs.add(colorMenu, 1, 1);
        
        GridPane.setConstraints(pieceLbl, 0, 0, 1, 1, HPos.RIGHT,
                VPos.TOP, Priority.NEVER, Priority.NEVER);
        
        buildButton.setOnAction(new EventHandler<ActionEvent>() {
            public void handle(ActionEvent e) {
                Controller.colorCopy((Piece)socket.getThing(), selectedColor, false);
                socket.clear();
    ...
    public void updatePreview() {
        ...
        if (piece != null) {
            Thing output = piece.getColoredCopy(selectedColor);
            preview.getChildren().add(output.getNode());
        }
    }
        
    public ImageView getIcon(int size) {
        return Novo.getImageView("images/" + size + "/palette_color.png", size);
    }
}
