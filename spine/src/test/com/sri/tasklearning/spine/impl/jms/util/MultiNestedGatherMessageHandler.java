...
public class MultiNestedGatherMessageHandler implements MessageHandler {
    private static final Logger log = LoggerFactory.getLogger(MultiNestedGatherMessageHandler.class);
    public JmsSpine jmsSpine;
    public Executor threadPool;

    public MultiNestedGatherMessageHandler(JmsSpine jmsSpine) {
    ...
    private class NestedTypeQueryGatherHandler implements Runnable {
        ...
        synchronized public void run() {
            ...
            if (sender.equals("Lumen")) {
                // Now that we have recieved the un-nested gather request we will issue a nested one
                // before responding.
                TypeQuery nestedTypeQuery = new TypeQuery(
                        jmsSpine.getClientId(),
                        (SimpleTypeName) TypeNameFactory
                                .makeName("nestedAction1"),
                        jmsSpine.getNextUid());
                try {
            ...
            // This must be the Lumen Instance.
                ...
                // Instead of responding to the bridge sub gather query, lets issue yet another
                // gather query!
                TypeQuery nestedTypeQuery = new TypeQuery(
                        jmsSpine.getClientId(),
                        (SimpleTypeName) TypeNameFactory
                                .makeName("nestedAction2"),
                        jmsSpine.getNextUid());
                try {
                    jmsSpine.gather(nestedTypeQuery, Spine.DEFAULT_TIMEOUT);
                    // Pretend to do something with the returned data
                ...
                // Now that we have recieved the nested2 gather request we will issue a response.
                SimpleTypeName name = (SimpleTypeName) TypeNameFactory
